...
Uansett hvilken man velger, er ikke-linearitet helt avgjørende for at et nodenettverk, tradisjonelle såvel som moderne, skal fungere. Uten dette blir alt forutsigbart og dødt. Men det ønskes nokså subtile endringer her, noe som ikke ødelegger vektingen, som kan beregnes effektivt og som enkelt kan deriveres (med tanke trening der gradientberegning inngår som en viktig del).
...
==== 5. Residual connection og Layer Norm (Stabilitet)
...
(om 50 000 tokens er definert). Hvert element i vektoren er skalarproduktet av radvektorene for hvert token i vokabular-matrisen og stem:[\mathbf{y^{(10)}}], og dette  korresponderer altså med vinkelen mellom dem. Store positive verdier indikerer at de peker i liknende retning.
...
AI-modeller av typen vi snakker om her trenes primært maskinelt med tapsfunksjon og backdrop som nevnt. Vi skal se på flere detaljer, men først nevne at de også trenes av mennesker, RLHF (Reinforcement Learning from Human Feedback). Dette inkluderer at mennesker vurderer og rangerer svar (hvilke som er gode, høflige, tydelig osv). En reward-modell utvikles og trener språkmodellen videre. Prosessen oppsummert kan sies å bestå av:

[source,text]
----
Pretraining (tekstdata)
        ↓
Supervised fine-tuning (menneskesvar)
        ↓
Reward model (menneskelig rangering)
        ↓
RL (optimaliser for menneskelig preferanse)
----

Det er mulig å fordype seg mer i dette for de som er interessert i disse sidene. Vi skal imidlertid heller se nærmere på backdrop-algoritmen.

Utgangspunktet er en tapsfunksjon. Man gjør en liten endring i hvordan modellen tar ut neste token. Isteden for å bare kopiere ut siste radvektor stem:[\mathbf{y^{(10)}}] fra output-matrisen, tar man ut alle stem:[\mathbf{y^{(1)}} \: \cdots \: \mathbf{y^{(T)}} ]. Dette gir stem:[T] sannsynlighetsvektorer 
stem:[\mathbf{p^{(1)}} \: \cdots \: \mathbf{p^{(T)}}] med elementer stem:[p_i^{(j)}] og en sekvens av tokens:
...
Modellen prøver å estimere:

[stem]
++++
P(\tau_t | \tau_1, \tau_2, \ldots, \tau_{t-1})
++++

altså sannsynligheten for neste tokens, gitt tidligere tokens.


Tapsfunksjonen defineres som:

[stem]
++++
\mathcal{L} = - \sum_{t=1}^{T} \log P(\tau_t \mid \tau_1, \dots, \tau_{t-1})
++++

altså vi summerer logaritmen av disse sannsynligheten fra det første til det siste. Det kan synes noe tilfeldig, men viser seg å forenkle de første stegene i gradient-beregningene vi trenger i backdrop. Den avsluttende stem:[\text{softmax}]-operasjonen av logits'ene får et enkelt uttrykk. Dessuten fungerer den som en tapsfunksjon. Riktig svar (riktig token) representeres av en vektor

[stem]
++++
\rho^{(i)} = [0, 0, \: \cdots \: 0, 1, 0, \: \cdots \: 0]
++++

og det kan vises at tapsfunksjonen vår kan skrives

[stem]
++++
\mathcal{L} = -\sum_{t=1}^T \sum_{k=1}^V \rho_k^t \: log \: p_k^t
++++

som gir veldefinerte verdier.

Backdrop består bare i å beregne gradienten til funksjonen stem:[\mathcal{L}] i vektorrommet av alle parametre stem:[w_i] i modellen (betraktet som variable), og i punktet bestemt av verdien alle disse parametrene har per nå. Dette kan sikkert virke både overveldende og forvirrende. Det er mye notasjon å holde orden på, men idéen er ganske enkel. La oss minne oss på matematikken her:

En (tilstrekkelig pen) funksjon stem:[f] i tre variable stem:[x, y, x], altså stem:[f(x, y, z)], har en gradientvektor stem:[\nabla f] gitt ved
...
som kan beregnes i alle (lovlige) punkter stem:[P = (x_0, y_0, z_0)] og peker der i retning av størst vekst (slik at at stem:[- \nabla f] peker i mest avtakende retning).

I vårt tilfelle har vi svært mange operasjoner og parametre stem:[w_i]. Men legg for det første merke til at operasjonene virker i tur og orden ved at output fra en operasjon eller lag, blir input til neste operasjon eller lag. Dette gir opphav til sammensatte funksjoner. F.eks.

[stem]
++++
\mathbf{X} \rightarrow Att_1 \rightarrow FFN_1 \rightarrow Att_2 \rightarrow FFN_2
++++

[stem]
++++
\mathbf{X} \rightarrow FFN_2\left( Att_2 \left( FFN_1 \left( Att_1(X) \right) \right) \right)
++++

Hver av disse kan deles videre opp i sammensatte funksjoner også, og det samme kan residual connections og layer-norm. Nå er det riktignok slik at tapsfunksjonen og gradienten opererer på enkeltvariable, som stem:[p_{k,t}] og stem:[w_i], ikke på matrise-likninger som våre nevnte. Men det er mulig å nyttiggjøre seg av matrix-calculus her, ved å utnytte Jacobi-matriser i likninger der det trengs. Arkitekturen er delvis valgt for å effektivisere gradientberegningen (mange av dem virker lineært, via diagonale matriser mm og er dermed jacobianere). Med rette betingelser vil:

[stem]
++++
\frac{\partial \mathcal{L}}{\partial\mathbf{M}} = 
\frac{\partial \mathcal{L}}{\partial\mathbf{M_1}} \cdot 
\frac{\partial\mathbf{M_1}}{\partial\mathbf{M}}
++++

(Betingelsene inkluderer at stem:[\frac{\partial \mathcal{L}}{\partial\mathbf{M_1}}] er gradient og stem:[\frac{\partial\mathbf{M_1}}{\partial\mathbf{M}}] er jacobian.)

Matrix-calculus er nok mer utilgjengelig for mange enn resten av matematikken. Men for å forstå beregning av en gradient, kan man ta utgangspunkt i tapsfunksjonen og _tenke_ seg at man nøster opp alt fra enkeltvariable i tapsfunksjonen. Dette blir i praksis umulig med det antallet parametre stem:[w_i] vi jobber med, men er mer på linje med en matematikk de fleste kjenner.

Dette ville blitt mange lag av sammensatte funksjoner, men dette er noe som er enkelt enkelt å derivere. Vi minner om kjerneregelen som sier at
...
Vi kan godt ha flere lag av sammensatte funksjoner her, og vi vet at denne prosessen stopper fint etter et antall derivasjonssteg. På tilsvarende måte vil utrykkene

[stem]
++++
\frac{\partial \mathcal{L}}{\partial w_i} = 
\frac{\partial \mathcal{L}}{\partial f} \cdot 
\frac{\partial f}{\partial g} \cdot 
\frac{\partial g}{\partial h} \cdots
\frac{\partial z}{\partial w_i}
++++

fungere og i prinsippet kunne beregnes (selv om arbeidsmengden her eksploderer). Dermed kan man sikkert godta at gradienten er veldefinert (selv om en faktisk beregning i praksis krever matrix-calculus).

Deretter man oppdaterer man alle parametre ved uttrykk som

[stem]
++++
\mathbf{w^{(i)}} \rightarrow \mathbf{w^{(i)}}  - \eta \: \nabla \mathcal{L} \: \mathbf{w^{(i)}} 
++++

der steglengden stem:[\eta] bestemmes mer eksperimentelt.

== Bildegenerering

Ikke ferdig

Følgende bildeillustrasjon viser forden AI-motoren tar utgangspunkt i et bilde med tilfeldige piksler og gradvis jobber fram et ferdig bilde.

image::images/picture_gen.png[800,600]
...
📘 https://mailjaro.github.io/linux-repo/[Linux for hjemmebruk]

📘 https://mailjaro.github.io/css-repo/[Litt om CSS: Virkemåte]

📘 https://mailjaro.github.io/git-repo/[Litt om Git]

📘 https://mailjaro.github.io/vscode-repo/[Litt om VS Code]

📘 https://mailjaro.github.io/gpg-repo/[Litt om GPG]

📘 https://mailjaro.github.io/highlight-repo/[Litt om syntaksutheving]

📘 https://mailjaro.github.io/makefile-repo/[Litt om Makefile]
