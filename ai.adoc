...
Merk at om bruker-spørsmålet bestod av stem:[10] tokens, så blir resultatet en stem:[10×10]-matrise (om vi ser bort fra eventuell padding). Dette må derfor ha noe med vår 10 input-tokens å gjøre. Under ser vi et eksempel på resultatmatrisen på dette stadiet i beregningen. Eller mer presist ville dette nøyaktig vært det vi hadde fått om vi multiplisert stem:[\mathbf{X}] og stem:[\mathbf{X^T}] direkte. Nå er radvektorene først endret av harde vektmatriser, dvs. de er flyttet lineært rundt i vektrommet og representerer ikke konkrete noe tokens med sine nåværende verdier (i prinsippet kunne de til og med ha representert andre tokens, selv om dette er ekstremt lite sannsynlig). Det virker likevel rimelig å si at dette er sammenhengen mellom token'ene etter radvektorenes ulike "reiser" i vektrommet bestemt av stem:[\mathbf{W^Q}] og stem:[\mathbf{W^K}].

Uansett er hvert tall resultat av skalarproduktet mellom en radvektor i stem:[\mathbf{Q}] og en radvektor i stem:[\mathbf{K}]. Tallet er altså relatert til vinkelen mellom vektorene, men ikke helt og fullt siden man ikke har dividert med vektorlengdene (og fått cosinus-vinkel). Tall kan bli relativt store både fordi vinkelen mellom dem er liten eller vektorene er lange f.eks. Negative verdier er fullt mulig her, men er utelatt av hensyn til pen, enkel formatering.
